	...
	if(is_admin() && current_user_can('activate_plugins') && !wp_doing_ajax()){
		...
		if(empty($spro_is_done) && empty($spro_is_dismissed)){
			...
			<div class="notice notice-error is-dismissible" id="softaculous-onboarding-notice">
				<p>'.sprintf(__('Your Onboarding process is not completed yet! <a href="%1$s" target="_blank">Click here</a> to complete the simple setup process and build your site in a few steps.', 'softaculous-pro'),admin_url('admin.php?page=assistant&act=onboarding')).'</p>
			</div>
