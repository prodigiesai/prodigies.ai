...
class EMagnificPopup extends CWidget {
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            ...
            Yii::app()->clientScript->registerScriptFile($baseUrl . '/jquery.magnific-popup' . (!YII_DEBUG ? '.min' : '') . '.js', CClientScript::POS_BEGIN);
            ...
            if ($this->language) {
                ...
                if (in_array($this->language, $avaliableLanguages)) {
                    Yii::app()->clientScript->registerScriptFile($baseUrl . '/locales/jquery.magnific-popup.' . $this->language . '.js', CClientScript::POS_BEGIN);
                } elseif (in_array(substr($this->language, 0, 2), $avaliableLanguages)) {
                    Yii::app()->clientScript->registerScriptFile($baseUrl . '/locales/jquery.magnific-popup.en.js', CClientScript::POS_BEGIN);
                }
            }
