...
class ExtEditMe extends CInputWidget {
	...
	public function run() {
		...
		if($this->onlyLoadAssets){
            Yii::app() -> clientScript -> registerScript('editMe', 'var WYSIWYG_CONF = ' . $ckeConfig . ';', CClientScript::POS_END);
        } else {
            if (empty($this -> inlineId)) {
                $jqSelector = CJavaScript::encode('#' . $this -> htmlOptions['id']);
                Yii::app() -> clientScript -> registerScript('editMe_' . $this -> htmlOptions['id'], 'var WYSIWYG_CONF = ' . $ckeConfig . '; jQuery(' . $jqSelector . ').ckeditor(' . $ckeConfig . ');', 2);
            } else {
                $selector = CJavaScript::encode($this -> inlineId);
                $jqSelector = CJavaScript::encode('#' . $this -> inlineId);
                Yii::app() -> clientScript -> registerScript('editMe_inline_' . $this -> inlineId, 'var WYSIWYG_CONF = ' . $ckeConfig . '; jQuery(' . $jqSelector . ').attr("contenteditable",true);CKEDITOR.disableAutoInline=true;CKEDITOR.inline(' . $selector . ',' . $ckeConfig . ');', 2);
            }
        }
