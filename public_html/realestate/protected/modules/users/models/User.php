...
class User extends ParentModel
{
    ...
    public function relations()
    {
        ...
        if (issetModule('payment')) {
            $relation['apartments'] = array(self::HAS_MANY, 'Apartment', 'owner_id');
        }
    ...
    public function rules()
    {
        $rules = array(
            ...
            array('username, email, agency_name, ava', 'length', 'max' => 100),
            array('password, salt, temprecoverpassword, api_token, activatekey, recoverPasswordKey', 'length', 'max' => 50),
            ...
            array('balance, type, agency_user_id, is_use_api', 'numerical', 'integerOnly' => true),
        ...
        $rules[] = array('active, agency_user_id, agent_status, date_created', 'safe');
    ...
    public function attributeLabels()
    {
        $return = array(
            ...
            'is_use_api' => tt('Use API', 'users'),
            'api_login' => tt('API user', 'users'),
            'api_password' => tt('API password', 'users'),
            'api_token' => tt('API token', 'users'),
        );
...
//		}
    ...
    public static function getTypeList($variant = 'list', $selected = null)
    {
        $list = array(
        ...
        if ($selected !== null) {
            return (isset($list[$selected])) ? $list[$selected] : '-';
        }
    ...
    {
        return CHtml::link(tt('all_member_listings', 'apartments') . ' (' . $this->countAd . ')', $this->getUrl());
    }
    ...
    public static function allowAdminEdit($user)
    {
        if ($user->role != User::ROLE_ADMIN && $user->role != User::ROLE_MODERATOR) {
            ...
            if (Yii::app()->user->role == User::ROLE_ADMIN && $user->role == User::ROLE_ADMIN) {
                return true;
            } elseif (Yii::app()->user->role == User::ROLE_MODERATOR && $user->role == User::ROLE_MODERATOR && Yii::app()->user->id != $user->id) {
                return false;
            } elseif ($user->role == User::ROLE_ADMIN) {
                return false;
            } elseif (Yii::app()->user->role == User::ROLE_MODERATOR && $user->role == User::ROLE_ADMIN) {
                return false;
            } elseif (Yii::app()->user->id == $user->id) {
                return true;
            } elseif ((Yii::app()->user->checkAccess("users_admin") && $user->role != User::ROLE_ADMIN) || ($user->role == User::ROLE_ADMIN && Yii::app()->user->checkAccess("admin"))) {
                return true;
            } else
                return false;
        }
    ...
    public static function allowAdminView($user)
    {
        if ($user->role != User::ROLE_ADMIN && $user->role != User::ROLE_MODERATOR) {
            return true;
        } else {
            if (Yii::app()->user->role == User::ROLE_ADMIN && $user->role == User::ROLE_ADMIN && Yii::app()->user->id == $user->id) {
                return true;
            } elseif (Yii::app()->user->role == User::ROLE_MODERATOR && $user->role == User::ROLE_MODERATOR && Yii::app()->user->id != $user->id) {
                return false;
            } elseif ($user->role == User::ROLE_ADMIN) {
    ...
    public static function allowAdminDelete($user)
    {
        if ($user->role != User::ROLE_ADMIN && $user->role != User::ROLE_MODERATOR) {
            return true;
        } else {
    ...
    public static function getUseApiOptions($selected = null)
    {
        $return[self::USE_API_YES] = tc('Yes');
        $return[self::USE_API_NO] = tc('No');
    ...
    public static function generateUserAPIToken($userId = null)
    {
        return md5(uniqid($userId . '_' . time() . '_' . Yii::app()->request->hostInfo . '_' . mt_rand(0, 1000)));
    }
}
