...
class CImageHandler extends CApplicationComponent
{
    ...
    private function freeImage()
    {
        ...
        if ($this->originalImage !== null) {
            if (is_resource($this->originalImage['image']) || $this->originalImage['image'] instanceof \GdImage) {
            ...
            switch ($result['format'] = $imageInfo[2]) {
                case self::IMG_GIF:
                    if ($result['image'] = @imagecreatefromgif($file)) {
                        ...
                        throw new Exception("{$file} - Invalid image gif format");
                    }
                    break;
                case self::IMG_JPEG:
                    if ($result['image'] = @imagecreatefromjpeg($file)) {
                        ...
                        throw new Exception("{$file} - Invalid image jpeg format");
                    }
                    break;
                case self::IMG_PNG:
                    if ($result['image'] = @imagecreatefrompng($file)) {
                        ...
                        throw new Exception("{$file} - Invalid image png format");
                    }
                    break;
                default:
    ...
    public function text($text, $fontFile, $size = 12, $color = array(0, 0, 0), $corner = self::CORNER_LEFT_TOP, $offsetX = 0, $offsetY = 0, $angle = 0, $alpha = 0)
    {
        ...
        if ($alpha > 0) {
            $color = imagecolorallocatealpha($this->image, $color[0], $color[1], $color[2], round($alpha));
        } else {
    ...
    public function adaptiveThumb($width, $height)
    {
        ...
        if ($widthProportion > $heightProportion) {
            $newWidth = $width;
            $newHeight = ceil($newWidth / $this->width * $this->height);
        } else {
            $newHeight = $height;
            $newWidth = ceil($newHeight / $this->height * $this->width);
        }
