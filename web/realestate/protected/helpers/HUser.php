...
class HUser
{
    ...
    public static function isAllowSendMessage(User $user)
    {
        return issetModule("messages")
            && Yii::app()->user->checkAccess("messages_admin")
            && Yii::app()->user->id != $user->id
            && $user->role == "registered" && $user->active == 1;
    }

    public static function isAdminOrModerator()
    {
        return Yii::app()->user->checkAccess('backend_access');
    }
}
