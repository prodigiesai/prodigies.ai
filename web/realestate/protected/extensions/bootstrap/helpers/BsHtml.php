...
EOD;
    ...
    public static function smenu(array $items, $htmlOptions = array(), $depth = 0)
    {
        // todo: consider making this method protected.
        if (!empty($items)) {
            $htmlOptions['role'] = 'menu';
            ...
            foreach ($items as $itemOptions) {
                ...
                if (is_string($itemOptions)) {
                    ...
                    $visible = BsArray::popValue('visible', $itemOptions);
                    if (!$visible) {
                        continue;
                    }
                    if (isset($itemOptions['visible']))
                        unset($itemOptions['visible']);
                    //HotFix 06.03.2014
                    if (is_array($itemOptions) && array_key_exists('itemOptions', $itemOptions)) unset( $itemOptions['itemOptions'] );
                    ...
                    $label = CHtml::openTag('span').BsArray::popValue('label', $itemOptions, '').CHtml::closeTag('span');
                    if (BsArray::popValue('active', $itemOptions, false)) {
                        static::addCssClass('active', $itemOptions);
                    }
                    if (BsArray::popValue('disabled', $itemOptions, false)) {
                    ...
                    if (BsArray::popValue('notify', $itemOptions, false)) {
                        static::addCssClass('notify', $itemOptions);
                    }
                    if (!isset($itemOptions['linkOptions'])) {
                        $itemOptions['linkOptions'] = array();
                    }
                    $icon = BsArray::popValue('icon', $itemOptions);
                    if (!empty($icon)) {
                        $label = static::icon($icon, array(), 'i') . ' ' . $label;
                    }
                    $bage = BsArray::popValue('bage', $itemOptions);
                    if (!empty($bage)) {
                        $label = $label.' '.$bage;
                    }
                    $items = BsArray::popValue('items', $itemOptions, array());
                    ...
                    if($items && static::isActiveInarray($items)){
                        static::addCssClass('active', $itemOptions);
                    }
                    ...
                    if ($divider) {
                        ...
                        $output .= static::menuTree($label, $url, $items, $itemOptions, $depth);
                    }
                }
            }
            $output .= '</ul>';
    ...
    public static function isActiveInArray($items) {
        if ($items && is_array($items)) {
            foreach ($items as $item) {
                if (is_array($item) && isset($item['active']) && $item['active'])
                    return true;
    ...
    protected static function menuTree($label, $url, $items, $htmlOptions, $depth = 0)
    {
        static::addCssClass($depth === 0 ? 'treeview' : 'treeview', $htmlOptions);
        $linkOptions = BsArray::popValue('linkOptions', $htmlOptions, array());
        $menuOptions = BsArray::popValue('menuOptions', $htmlOptions, array());
        static::addCssClass('treeview-menu', $menuOptions);
        if ($depth === 0) {
            $defaultId = parent::ID_PREFIX . parent::$count++;
            BsArray::defaultValue('id', $defaultId, $menuOptions);
            $menuOptions['aria-labelledby'] = $menuOptions['id'];
        ...
        $output = static::openTag('li', $htmlOptions);
        $output .= static::treeToggleMenuLink($label, $url, $linkOptions, $depth);
        $output .= static::smenu($items, $menuOptions, $depth + 1);
        $output .= '</li>';
        return $output;
    }
    ...
    public static function treeToggleMenuLink($label, $url = '#', $htmlOptions = array(), $depth = 0)
    {
    ...
    public static function sidemenu($type, $items, $htmlOptions = array())
    {

        if (!empty($type)) {
            static::addCssClass($type.'-menu', $htmlOptions);
        } else
            static::addCssClass('sidebar-menu', $htmlOptions);

        return static::smenu($items, $htmlOptions);
    }
