...
function softaculous_pro_ajax_start_install_template(){
	...
	if($template['type'] > 1 && empty($softaculous_pro['license']['active'])){
		$data['error']['template_pro'] = sprintf(__('The selected template is a Pro template and you have a free or expired license. Please enter your license key <a href="%1$s" target="_blank" style="color:blue;">here</a>.', 'softaculous-pro'), admin_url('admin.php?page=assistant&act=license'));
		softaculous_pro_ajax_output($data);
	}
	...
	if(!function_exists('pagelayer_theme_import_notices') || (empty($do_we_have_pro) && $template['type'] > 1)){
		...
		if(is_wp_error($installed) || empty($installed)){
			...
			$data['error']['pagelayer'] = sprintf(__('There was an error in installing Pagelayer which is required by this template. Please install Pagelayer manually by clicking <a href="%1$s" target="_blank">here</a> and then install the template !', 'softaculous-pro'), $install_url);
	...
	<div class="softaculous-pro-modal-holder">
		...
		<div class="softaculous-pro-modal-content">
			...
			<div class="softaculous-pro-done" style="display: block;">
				...
				<?php _e('You can now customize the website as per your requirements with the help of Pagelayer or the Customizer.<br><br>
				<b>Note</b> : We strongly recommend you change all images and media. We try our best to use images which are copyright free or are allowed under their licensing. However, we take no responsibilities for the same and recommend you change all media and images !', 'softaculous-pro'); ?>
			</div>
		</div>
